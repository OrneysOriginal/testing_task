...
async def shortener_url(url: SUrl, session: AsyncSession = Depends(get_async_session)) -> dict:
...
@shortener_router.get('/<shorten-url-id>')
async def get_original_url(url_id: int, session: AsyncSession = Depends(get_async_session)) -> dict:
